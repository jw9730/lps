...
class VNBatchNorm(nn.Module):
    def __init__(self, num_features, dim):
        ...
        # this is not in the original vnn implementation but added for lps
...
class VNMaxPool(nn.Module):
    ...
    def forward(self, x):
        ...
        idx = dotprod.max(dim=-1, keepdim=False)[1].to(x.device)
        index_tuple = torch.meshgrid([torch.arange(j, device=x.device) for j in x.size()[:-1]]) + (idx,)
        x_max = x[index_tuple]
        return x_max
